...
function CreatePrehistoricAnimal(){
    ...
    const handleSubmit = async (event) => {
        ...
        try{
            ...
            data.append("prehistoricAnimal", formData.video);
    ...
    return(
        <form onSubmit={handleSubmit} encType="multipart/form-data" className="max-w-xl mx-auto bg-white p-6 rounded-2xl border border-slate-800 shadow-sm flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-1">
                <label htmlFor="video" className={"text-sm font-semibold text-slate-700"}>visual</label>
                <input
                    type="file"
                    id="video"
                    name="prehistoricAnimal"
                    accept="video/mp4"
                    className="rounded-lg border border-slate-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent"
                    onChange={(e)=>
                setFormData({
